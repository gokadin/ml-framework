...
extern "C" {
    ...
    __declspec(dllexport) int gpu_mul_forward(TENSOR *target, TENSOR* a, TENSOR *b) {
        ...
        dim3 gridSize = dim3((target->mat_size + BLOCK_SIZE - 1) / BLOCK_SIZE);
        mul<<<gridSize, blockSize>>>(gpu_a, gpu_b, gpu_target, target->mat_size);
    ...
    __declspec(dllexport) int gpu_mul_backward(TENSOR *target, TENSOR* a, TENSOR *b) {
        ...
        size_t tensor_grad_size = target->grad_size * sizeof(float);
        ...
        dim3 blockSize = dim3(BLOCK_SIZE);
        dim3 gridSize = dim3((target->mat_size + BLOCK_SIZE - 1) / BLOCK_SIZE);
        mul_grad<<<gridSize, blockSize>>>(gpu_a, gpu_a_grad, gpu_b, gpu_b_grad, gpu_tensor_grad, target->grad_size);
        checkCudaKernelErr("add", blockSize, gridSize);
