...
extern "C" {
    ...
    __declspec(dllexport) void gpu_softmax_forward(TENSOR *target, TENSOR *a) {
        ...
        dim3 gridSize = dim3((a->mat_shape[1] + BLOCK_SIZE - 1) / BLOCK_SIZE, (a->mat_shape[0] + BLOCK_SIZE - 1) / BLOCK_SIZE);
        softmax<<<gridSize, blockSize>>>(gpu_a, gpu_target, a->mat_shape[0], a->mat_shape[1]);
