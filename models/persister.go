...
func saveModel(model *Model, name string) {
	...
	if err != nil {
		log.Fatal("Could not marshal model")
	}

	ioutil.WriteFile(fmt.Sprintf("%s/%s", modelStoreRoot, name), data, 0644)
}
...
func Restore(name string) *Model {
	...
	file, _ := ioutil.ReadFile(fmt.Sprintf("%s/%s", modelStoreRoot, name))
	model := &Model{}
	json.Unmarshal(file, &model)
	return model
}

//func Restorew(name string) *Model {
//	_, err := os.Stat(fmt.Sprintf("%s/%s", modelStoreRoot, name))
//	if os.IsNotExist(err) {
//		log.Fatalf("model %s does not exist", name)
//	}
